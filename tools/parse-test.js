...
/* Смуга мініатюр під верхнім рядом: 8,74 × 1,80. Пропорції фото відомі. */
let bg = P.bestGrid([0.75, 0.75, 0.75, 0.75], 8.74, 1.80);
eq([bg.cols, bg.rows], [4, 1], '4 вертикальні фото у смузі → в один ряд по 4');
bg = P.bestGrid([4 / 3, 4 / 3, 4 / 3, 4 / 3, 4 / 3, 4 / 3, 4 / 3, 4 / 3, 4 / 3, 4 / 3, 4 / 3, 4 / 3, 4 / 3], 8.74, 1.80);
ok(bg.cols >= 5 && bg.rows === 3 || bg.rows === 2, '13 горизонтальних у смузі → 2–3 ряди', bg.cols + '×' + bg.rows);
bg = P.bestGrid([0.75, 0.75, 0.75, 0.75], 8.74, 4.55);
eq([bg.cols, bg.rows], [4, 1], '4 вертикальні у великій області → 4 в ряд (а не 2×2, як для горизонтальних)');
const mixed = P.bestGrid([4 / 3, 0.75, 4 / 3], 8.74, 1.80);
ok(mixed.cols === 3 && mixed.rows === 1, 'мішані пропорції: три в ряд', mixed.cols + '×' + mixed.rows);
